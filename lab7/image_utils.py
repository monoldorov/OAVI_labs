...
def save_binary_png(binary_black: np.ndarray, output_path: Path) -> None:
    ...
    Image.fromarray(image_data, mode="L").save(output_path)
...
def render_symbol_to_binary(
    ...
    if int(binary_black.sum()) == 0:
        raise ValueError(f"После бинаризации символ {symbol!r} пустой.")

    return binary_black


def generate_reference_symbols(
    alphabet: list[str],
    font,
    canvas_size: tuple[int, int],
    threshold: int,
    ...
    for symbol in alphabet:
        binary = render_symbol_to_binary(symbol, font, canvas_size, threshold)
...
def build_phrase_image_from_symbols(
    ...
    if not symbol_imgs:
        raise ValueError("Фраза не содержит символов.")
...
def save_crop(binary_black: np.ndarray, box: tuple[int, int, int, int], output_path: Path) -> None:
    x1, y1, x2, y2 = box
